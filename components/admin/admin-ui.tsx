...
export async function readApi<T>(response: Response): Promise<T> {
  ...
  if (response.status === 403 && body.error?.code === "PERMISSAO_NEGADA") {
    window.location.replace("/painel?acesso=negado");
...
export function StatusBadge({ status }: { status: string }) {
  const active = status === "ATIVO" || status === "ATIVA" || status === "MEMBRO_ATIVO" || status === "CONGREGADO" || status === "NOVO_CONVERTIDO" || status === "PAGA" || status === "ISENTA" || status === "SUCESSO";
  const warning = status === "BLOQUEADO" || status === "SEGURANCA" || status === "PENDENTE" || status === "TESTE" || status === "VISITANTE" || status === "AFASTADO" || status === "EM_CARENCIA" || status === "AGUARDANDO_PAGAMENTO" || status === "VENCIDA";
  return (
