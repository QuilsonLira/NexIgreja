...
function formatLastAccess(session: SafeSessionPayload): string {
  ...
  const formatted = new Intl.DateTimeFormat("pt-BR", {
    ...
    hour: "2-digit",
    minute: "2-digit"
  }).format(new Date(access.dateTime));
...
export function DashboardShell() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (sessionResponse.status === 401 || contextsResponse.status === 401) {
        ...
        if (!sessionResponse.ok || !contextsResponse.ok || !sessionBody.session || !contextsBody.contexts) {
          throw new Error("Falha ao carregar o contexto");
        }
        if (active) {
        ...
        if (active) setFeedback("Não foi possível carregar sua sessão. Atualize a página.");
      } finally {
  ...
  const visibleBranches = useMemo(
    () => contexts?.branches.filter((branch) => branch.matrixId === selectedMatrixId) ?? [],
    [contexts, selectedMatrixId]
  );
  ...
  async function applyContext(matrixId: number, branchId: number | null) {
    ...
    try {
      ...
      if (!response.ok || !body.session) {
        setFeedback(body.error?.message || "Não foi possível trocar a unidade.");
        return;
      }
      setSession(body.session);
      setFeedback("Unidade de trabalho atualizada.");
    } catch {
      setFeedback("Não foi possível conectar ao sistema.");
    } finally {
    ...
    <div className="app-shell">
      <aside className={`sidebar${mobileMenu ? " sidebar-open" : ""}`}>
        ...
        <nav className="sidebar-nav" aria-label="Navegacao principal">
          <a className="nav-item nav-item-active" href="/painel" aria-current="page">
        ...
        <div className="sidebar-security">
          ...
          <div>
            ...
            <span>Dados limitados ao seu acesso</span>
          </div>
        </div>
      </aside>
      ...
      <div className="app-main">
        <header className="topbar">
          ...
          <div className="topbar-context">
            ...
            <strong>{session.activeContext?.unitName ?? "Selecione uma matriz"}</strong>
          </div>
        ...
        <main className="dashboard-content">
          <section className="welcome-row">
            <div>
              ...
              <h1>Ola, {session.user.name.split(" ")[0]}.</h1>
              <p>Seu acesso está pronto e respeita o alcance definido para sua conta.</p>
            </div>
          ...
          {session.user.mustChangePassword ? (
            <button className="security-banner" type="button" onClick={() => setPasswordOpen(true)}>
              <KeyRound size={22} />
              <span>
                <strong>Troque sua senha temporária</strong>
                Essa etapa é obrigatória antes de continuar.
              </span>
          ...
          <section className="dashboard-grid">
            <article className="content-card context-card">
              <div className="card-heading">
                ...
                <div>
                  <p className="eyebrow">Unidade de trabalho</p>
                  <h2>Escolha onde deseja trabalhar</h2>
                </div>
              </div>

              {session.user.organizationalScope === "FILIAL" ? (
                <div className="fixed-context">
                  <span>Unidade vinculada</span>
                  <strong>{session.activeContext?.unitName}</strong>
                  <small>A troca de unidade não está disponível para este acesso.</small>
                </div>
              ) : (
                <div className="context-fields">
                  <div className="field-group">
                    <label htmlFor="matrix-select">Matriz</label>
                    <select
                      id="matrix-select"
                      value={selectedMatrixId ?? ""}
                      disabled={!contexts.canChangeMatrix || changingContext || session.user.mustChangePassword}
                      onChange={(event) => {
                        const matrixId = Number(event.target.value);
                        if (matrixId) void applyContext(matrixId, null);
                      }}
                    >
                      <option value="">Selecione uma matriz</option>
                      {contexts.matrices.map((matrix) => (
                        <option key={matrix.id} value={matrix.id}>{matrix.name}</option>
                      ))}
                    </select>
                  </div>
                  <div className="field-group">
                    <label htmlFor="branch-select">Unidade</label>
                    <select
                      id="branch-select"
                      ...
                      onChange={(event) => {
                        ...
                        void applyContext(selectedMatrixId, branchId);
                      }}
                    >
                      <option value="">Trabalhar na própria matriz</option>
              ...
              <div className={`inline-feedback${feedback ? " inline-feedback-visible" : ""}`} aria-live="polite">
                {changingContext ? <LoaderCircle className="spin" size={16} /> : null}
                {changingContext ? "Atualizando unidade..." : feedback}
              </div>
            </article>
            ...
            <article className="content-card access-card">
              <div className="card-heading">
                ...
                <div>
                  ...
                  <h2>Ultimo acesso anterior</h2>
                </div>
              </div>
              <p className="last-access-text">{formatLastAccess(session)}</p>
              <div className="access-actions">
                <button className="text-button" type="button" onClick={() => setPasswordOpen(true)}>
                  <KeyRound size={18} />
                  Trocar minha senha
                </button>
                <button className="text-button text-button-danger" type="button" onClick={logout}>
                  <LogOut size={18} />
                  Sair do sistema
                </button>
              </div>
          ...
          <section className="content-card foundation-card">
            <div className="foundation-icon"><ShieldCheck size={25} /></div>
            <div>
              <p className="eyebrow">Primeira etapa</p>
              <h2>Base de acesso configurada</h2>
              <p>
                Login, sessão, auditoria e isolamento das unidades estão preparados. Os módulos operacionais serão adicionados nas próximas etapas.
              </p>
            </div>
            <div className="foundation-status"><span /> Proteção ativa</div>
          </section>
        </main>
      </div>
