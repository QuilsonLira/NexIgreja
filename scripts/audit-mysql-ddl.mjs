...
// Numeric IDs generated by NexIgreja can be around 1.7e15 and therefore must
// not remain as MySQL INT. Textual IDs (UUID/session/provider identifiers) are
// intentionally VARCHAR and are excluded from this numeric invariant.
for (const [tableName, table] of tables) {
  for (const [columnName, type] of table.columns) {
    ...
    const isNarrowInteger = /\b(?:TINYINT|SMALLINT|MEDIUMINT|INT|INTEGER)\b/.test(type) && !/\bBIGINT\b/.test(type);
    if (idLike && isNarrowInteger) {
      failures.push(`${tableName}.${columnName} cannot use narrow integer type, generated ${type}`);
    }
  }
}
