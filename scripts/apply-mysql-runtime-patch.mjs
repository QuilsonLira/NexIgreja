...
const mysqlCompatPath = path.join(process.cwd(), "lib", "server", "mysql-d1-compat.ts");
...
let mysqlCompatSource = await readFile(mysqlCompatPath, "utf8");
const legacyBackendSelector = `export function isMysqlBackendConfigured(): boolean {\n  const enabled = /^(1|true|yes|on)$/i.test(process.env.MYSQL_RUNTIME_ENABLED?.trim() ?? "");\n  if (!enabled) return false;\n  const value = process.env.DATABASE_URL?.trim() ?? "";\n  return value.startsWith("mysql://") || value.startsWith("mysql2://");\n}`;
const explicitBackendSelector = `export function isMysqlBackendConfigured(): boolean {\n  const enabled = /^(1|true|yes|on)$/i.test(process.env.MYSQL_RUNTIME_ENABLED?.trim() ?? "");\n  if (!enabled) return false;\n\n  const hasSeparateConfig = Boolean(\n    process.env.DB_HOST?.trim() ||\n    process.env.DB_USER?.trim() ||\n    process.env.DB_PASSWORD !== undefined ||\n    process.env.DB_NAME?.trim(),\n  );\n  if (hasSeparateConfig) return true;\n\n  const value = process.env.DATABASE_URL?.trim() ?? "";\n  return value.startsWith("mysql://") || value.startsWith("mysql2://");\n}`;

if (!mysqlCompatSource.includes(explicitBackendSelector)) {
  if (!mysqlCompatSource.includes(legacyBackendSelector)) {
    throw new Error("mysql-d1-compat.ts backend selector anchor not found");
  }
  mysqlCompatSource = mysqlCompatSource.replace(legacyBackendSelector, explicitBackendSelector);
  await writeFile(mysqlCompatPath, mysqlCompatSource, "utf8");
}

console.log("MySQL runtime patch applied to auth and backend selector");
