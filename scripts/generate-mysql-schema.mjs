...
// SQLite stores booleans as INTEGER with a mode flag. Convert those first so
// the remaining INTEGER columns can safely map 1:1 to signed BIGINT.
source = source.replace(
...
// SQLite INTEGER is a signed 64-bit integer. Mapping every remaining numeric
// INTEGER to MySQL BIGINT preserves the source range exactly and guarantees
// compatible types on foreign-key pairs (including columns such as created_by
// that do not end in _id). NexIgreja IDs are still below Number.MAX_SAFE_INTEGER.
source = source.replace(
  /\binteger\(\s*(["'])([^"']+)\1\s*\)/g,
  (_match, quote, columnName) =>
    `bigint(${quote}${columnName}${quote}, { mode: "number" })`,
);

// Fail loudly if any non-boolean SQLite INTEGER builder remains. This catches
// uncommon mode/config shapes instead of silently producing a narrower type.
if (/\binteger\s*\(/.test(source)) {
  throw new Error("Unconverted SQLite integer() call remains in generated schema");
}
...
// Fail loudly if any SQLite-only blob builder remains.
