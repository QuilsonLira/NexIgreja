...
// NexIgreja generates many identifiers with Date.now() * 1000 + random suffix,
// currently around 1.7e15. MySQL INT cannot store those values. Keep IDs,
// foreign keys and monetary cents as signed BIGINT in number mode; they remain
// below JavaScript's Number.MAX_SAFE_INTEGER and preserve the existing API.
source = source.replace(
  /\bint\(\s*(["'])([^"']+)\1\s*\)/g,
  (match, quote, columnName) => {
    const needsBigInt =
      columnName === "id" ||
      columnName.endsWith("_id") ||
      columnName.endsWith("_cents");
    if (!needsBigInt) return match;
    return `bigint(${quote}${columnName}${quote}, { mode: "number" })`;
  },
);
...
// IDs/FKs and monetary cents use BIGINT to preserve NexIgreja generated IDs.
